...
- Logs page UI/UX - add option to query by date using calendar drawer, pagination and pagination preferance 6,8,10,12,16,20. Add logs tooltip. Add option to select more workouts and delete them all at once on current, archived and logs pages._
...
- _Add email verification and option to get email from profile page, password reset option, option to delete profile with cascading, uploading profile picture. Add option in user preference for padding bottom on navbars._

---

### Future

- _Upgrade to tailwind v4 when ready. Persist form data from workout to do page inside local storage as backup when app crashes while doing a workout. Later on add cron job that removes all workouts older than 1 year if storage becomes the problem. Add gradient on landing page and make small svg "note your sets"._
