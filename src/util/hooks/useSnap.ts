/** biome-ignore-all lint/style/noNonNullAssertion: missing box properties are handled*/
import type {
  BoundingBox,
  DragElastic,
  DragHandler,
...
export type UseSnapResult = {
  dragProps: Pick<
    MotionProps,
    "drag" | "onDragStart" | "onDragEnd" | "onMeasureDragConstraints" | "dragMomentum"
  > &
...
export const useSnap = ({
  ...
  const onDragEndHandler: DragHandler = (event, info) => {
    ...
    const distances = points.map((p) => {
      if (p.x !== undefined && p.y !== undefined) {
        return Math.sqrt((p.x - afterInertia.x) ** 2 + (p.y - afterInertia.y) ** 2)
      }
  ...
  const dragProps: Partial<MotionProps> = {
    drag: direction === "both" ? true : direction === "none" ? false : direction,
