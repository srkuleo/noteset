...
import { type DirectionOptions, useSnap } from "@/util/hooks/useSnap"
...
const SwipeActionsContext = createContext({} as SwipeActionsContextProps)
  ...
  return (
    <SwipeActionsContext.Provider
      value={{
        actionsWidth,
        setActionsWidth,
        triggerRef: mergeRefs([handleRef, triggerMeasureRef]) as Ref<HTMLDivElement>,
        dragProps,
...
const Actions = ({
  ...
  return (
    <motion.div
      className={twMerge("absolute flex justify-end overflow-hidden", wrapperClassName)}
      style={{
